    ...
    if in_attributes:
        ...
        for face in data:
            ...
            if face_attributes_params != None:
                attributes += ReadFunctions.get_face_data(face['faceAttributes'])
                
                auxiliar = attributes.split('.\n')
                
                n_attributes += len(auxiliar)
            
            draw.rectangle((dimensions[0][0], dimensions[1][1], dimensions[0][0]+90, dimensions[1][1]+13*n_attributes),
                           fill= "red")
